    ...
    for (int i = 0; i < maze->height; i++)
    {
        for (int j = 0; j < maze->width; j++)
        {
            ...
            else
            {
                ...
                if (generate_random(0, 9) < 2) // Adjust the threshold for more or fewer obstacles
                {
 ...
 * @param maze
 */
void create_hardcoded_map(maze_t *maze)
{
...
void combined_task(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        mapping_direction_t moveDirection
            = (mapping_direction_t)(get_rand_32() % 4); // Randomly choose a direction
        ...
        vTaskDelay(pdMS_TO_TICKS(100)); // Delay to simulate time between movements
    }
}
 ...
 * @param maze
 */
void mapping_tasks_init(maze_t *maze)
{
